...
def main():
    ...
    try:
        ...
        st.error("Data file not found. Please ensure 'data/nobel_prizes_1901-2025_cleaned.csv' exists.")
        st.stop()
    ...
    # Apply filters
    filtered_df = filter_data(df, year_range, selected_categories, gender_filter, country_filter)
    ...
    tab1, tab2, tab3 = st.tabs(["📋 Data Table", "📈 Statistics", "🌍 Geographic Analysis"])
    
    with tab1:
        st.subheader("Filtered Nobel Prize Data")
        
        # Display key columns
        display_cols = ['award_year', 'known_name', 'category', 'sex', 'birth_country', 'motivation']
    ...
    with tab2:
        ...
        gender_by_year = filtered_df.groupby(['award_year', 'sex']).size().reset_index(name='count')
        fig3 = px.line(
            gender_by_year,
            x='award_year',
            y='count',
    ...
    with tab3:
        ...
        country_data = filtered_df['birth_country'].value_counts().head(20).reset_index()
        country_data.columns = ['country', 'count']
        
        st.write(f"**Top 20 Countries by Laureate Count**")
        st.dataframe(country_data, use_container_width=True)
        
        # Category breakdown by top 5 countries
        top_5_countries = filtered_df['birth_country'].value_counts().head(5).index.tolist()
        top_countries_df = filtered_df[filtered_df['birth_country'].isin(top_5_countries)]
    ...
    # Footer
    st.markdown("---")
    st.caption("Data source: Nobel Prize dataset (1901-2025) | Built with Streamlit")
