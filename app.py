...
def main():
    ...
    countries = ['All'] + sorted(
        df['birth_country'].dropna().unique().tolist()
    )
    ...
    with tab1:
        ...
        # Display key columns
        display_cols = [
            'award_year', 'known_name', 'category',
            'sex', 'birth_country', 'motivation'
        ]
        available_cols = [
    ...
    with tab2:
        ...
        gender_by_year = filtered_df.groupby(
            ['award_year', 'sex']
        ...
        if 'is_shared' in filtered_df.columns:
            ...
            with col2:
                ...
                decade_counts = filtered_df_copy.groupby(
                    'decade'
                ).size().reset_index(name='count')
    ...
    with tab3:
        ...
        country_data = filtered_df['birth_country'].value_counts().head(
            20
        ).reset_index()
        ...
        top_5_countries = filtered_df['birth_country'].value_counts().head(
            5
        ).index.tolist()
        ...
        category_country = top_countries_df.groupby(
